...
% zHx = zprop(1):dz:zprop(end);     % UNUSED VARIABLE (A. Tzanis, 2013)
% xHz = xprop(1):dx:xprop(end);     % UNUSED VARIABLE (A. Tzanis, 2013)
...
for s=1:nsrc;                                   
    [~,srci(s)] = min(abs(xEy-srcloc(s,1))); % source x index in Ey field matrix
    [~,srcj(s)] = min(abs(zEy-srcloc(s,2))); % source z index in Ey field matrix
    srcx(s) = xEy(srci(s));                     % true source x location
    srcz(s) = zEy(srcj(s));                     % true source z location
end
for r=1:nrec;                                   
    [~,reci(r)] = min(abs(xEy-recloc(r,1))); % receiver x index in Ey field matrix
    [~,recj(r)] = min(abs(zEy-recloc(r,2))); % receiver z index in Ey field matrix
...
%         li = l((l>lpmlTin & l<lpmlBin));        % UNNECESSARY VARIABLE (A. Tzanis, 2013)  
        ip = kp./2;  jp = (lp+1)./2;            % Hx indices in PML region
        ii = ki./2;                             % Hx indices in interior (non-PML) region
%         ji = (li+1)./2;                         % UNNECESSARY VARIABLE (A. Tzanis, 2013)
...
%         li = l((l>lpmlTin & l<lpmlBin));        % UNNECESSARY VARIABLE (A. Tzanis, 2013)
        ...
        ii = (ki+1)./2;                         % Hz indices in interior (non-PML) region
%         ji = li./2;                             % UNNECESSARY VARIABLE (A. Tzanis, 2013)
...
%         li = l((l>lpmlTin & l<lpmlBin));        % UNNECESSARY VARIABLE (A. Tzanis, 2013)
        ...
        ii = ki./2;                             % Ey indices in interior (non-PML) region
%         ji = li./2;                             % UNNECESSARY VARIABLE (A. Tzanis, 2013)
