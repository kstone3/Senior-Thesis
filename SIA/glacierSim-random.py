...
class glacierSim():
    ...
    def update_b(self, timestep):
        #fix nan temps(its probably using 0 for nan temps which is way wrong)
    ...
    def run_model(self,i):
        if i==0 and not self.initial_run:
            ...
            ax.set_title('Year = ' + str(self.current_date.year))
...
class GlacierEstimator(BaseEstimator, RegressorMixin):
    """
    A simple scikit-learn-compatible estimator that calls your 'optimize' function.
    We treat it like a 'Regressor' for convenience, but it's really just returning
    a cost measure via the 'score()' method.
    """
    def __init__(self, ice_meltfactor=-0.5,snow_meltfactor=-0.5,accumfactor=0.5,snow_conv_factor=5,snow_melt_amplitude=-0.5,ice_melt_amplitude=-0.5,opt_var='annual'):
        self.ice_meltfactor = ice_meltfactor
        self.snow_meltfactor = snow_meltfactor
        self.accumfactor = accumfactor
        self.snow_conv_factor = snow_conv_factor
        self.snow_melt_amplitude = snow_melt_amplitude
        self.ice_melt_amplitude = ice_melt_amplitude
        self.opt_var = opt_var
        self._cost_ = None  # Will hold the latest cost after 'fit'

    def fit(self, X, y=None):
        input_params = [self.ice_meltfactor,self.snow_meltfactor,self.accumfactor,self.snow_conv_factor,self.snow_melt_amplitude,self.ice_melt_amplitude]
        cost = optimize(self.opt_var, input_params)
        self._cost_ = cost
        return self

    def predict(self, X):
        return np.full(shape=(len(X),), fill_value=self._cost_ if self._cost_ is not None else 0.0)

    def score(self, X, y=None):
        if self._cost_ is None:
            return -1e10  # If somehow called before fit
        return -self._cost_
    
class SingleSplitCV(BaseCrossValidator):
    def get_n_splits(self, X=None, y=None, groups=None): return 1
...
param_distributions = {
    'ice_meltfactor':      uniform(loc=-1, scale=1),  # from -1 to 0
    'snow_meltfactor':     uniform(loc=-1, scale=1),  # from -1 to 0
    'accumfactor':         uniform(loc=0,  scale=1),  # from 0  to 1
    'snow_conv_factor':    uniform(loc=0,  scale=15), # from 0  to 15
    'snow_melt_amplitude': uniform(loc=-1, scale=1),  # from -1 to 0
    'ice_melt_amplitude':  uniform(loc=-1, scale=1),  # from -1 to 0
}
# gamma=0.0065
# bounds=[(0.005,0.007)]
...
with open(f"Results/{opt_method}-Results.txt", "a") as file:
    for opt_var in ['annual','ela','extent','front_var','thick','vol_change']:
        ...
        print("Optimized accumulation factor:", best_params['accumfactor'])
        print("Optimized ice meltfactor:",best_params['ice_meltfactor'])
        print("Optimized snow meltfactor:",best_params['snow_meltfactor'])
        print("Optimized snow conversion factor:",best_params)
        print("Optimized snow melt amplitude:",best_params['snow_melt_amplitude'])
        print("Optimized ice melt amplitude:",best_params['ice_melt_amplitude'])
        print("Final objective function value:", random_search.best_score_)
        print(best_params)
        file.write(f"Optimized parameters for {opt_var}:\n")
        file.write(f"Optimized accumulation factor: {best_params['accumfactor']}\n")
        file.write(f"Optimized ice meltfactor: {best_params['ice_meltfactor']}\n")
        file.write(f"Optimized snow meltfactor: {best_params['snow_meltfactor']}\n")
        file.write(f"Optimized snow conversion factor: {best_params['snow_conv_factor']}\n")
        file.write(f"Optimized snow melt amplitude: {best_params['snow_melt_amplitude']}\n")
        file.write(f"Optimized ice melt amplitude: {best_params['ice_melt_amplitude']}\n")
        file.write(f"Final objective function value: {random_search.best_score_}\n")
