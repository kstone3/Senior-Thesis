...
def initIndividual(icls, bounds):
    individual = []
    for (low, high) in bounds: individual.append(random.uniform(low, high))
    return icls(individual)

def initPopulation(pcls, icls, bounds, size=50): return pcls(initIndividual(icls, bounds) for _ in range(size))
def evalGlacier(individual, opt_var): return (optimize(opt_var, individual),)  # DEAP requires a tuple

toolbox = base.Toolbox()
toolbox.register("individual", initIndividual, creator.Individual, bounds)
toolbox.register("population", initPopulation, list, creator.Individual, bounds)
...
with open(f"Results/{opt_method}-Results.txt", "a") as file:
    for opt_var in ['annual','ela','extent','front_var','thick','vol_change']:
        ...
        def evaluate_individual(ind): return evalGlacier(ind, opt_var)
        toolbox.register("evaluate", evaluate_individual)
        for gen in range(20):
            offspring = algorithms.varAnd(population, toolbox, cxpb=0.5, mutpb=0.2)
            for ind, fit_val in zip(offspring, toolbox.map(toolbox.evaluate, offspring)): ind.fitness.values = fit_val
            population = toolbox.select(offspring, k=len(population))

        best_ind = tools.selBest(population, 1)[0]
        best_cost = best_ind.fitness.values[0]

        (result_ice_melt,result_snow_melt,result_accum,result_snow_conv,result_snow_amp,result_ice_amp) = best_ind
        ...
        print("Final objective function value:", best_cost)
        print(best_ind)
        ...
        file.write(f"Final objective function value: {best_cost}\n")
        file.write(f"{best_ind}\n")
        file.flush()
